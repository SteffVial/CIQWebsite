function Get-Tree {
    ...
    Get-ChildItem -Path $Path -Directory -Force | Where-Object { $_.Name -ne "node_modules" } | ForEach-Object {
        "$prefix$($_.Name)" | Out-File -Append backendStructure.txt
...
Remove-Item backendStructure.txt -ErrorAction SilentlyContinue

# Ajouter un titre en haut du fichier
"-----backend-----" | Out-File -FilePath backendStructure.txt

# Lancer l'analyse sur le dossier voulu
Get-Tree -Path "C:\VSCODE\CIQWebsite\backend"
